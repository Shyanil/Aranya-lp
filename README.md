...
Both the main enquiry modal and coming-soon form POST to `/api/leads`. The server forwards form fields, page URL, form source, UTM fields, and server-generated `date` (`dd/mm/yy`), `time` (`HH:mm:ss`, 24-hour), `submitted_at` (date and time), and `timezone` (`Asia/Kolkata`). Success is displayed only after the webhook returns a successful HTTP status.
