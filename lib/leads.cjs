function kolkataTimestamp(now) {
  ...
  const period = String(parts.dayPeriod || '').replace(/\./g, '').replace(/\s/g, '').toUpperCase();
  const time = `${parts.hour}:${parts.minute}:${parts.second} ${period.includes('P') ? 'PM' : 'AM'}`;
...
async function handleLead(method, body, { webhookUrl = process.env.PABBLY_WEBHOOK_URL, fetchImpl = fetch, now = new Date() } = {}) {
  ...
  lead.phone = localPhoneDigits(lead.phone);
  if (!lead.name || !/^\d{10}$/.test(lead.phone) || (lead.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(lead.email)) || (lead.pincode && !/^\d{6}$/.test(lead.pincode)) || !['main_enquiry', 'coming_soon'].includes(lead.form_source)) {
